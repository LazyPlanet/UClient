...
public static class ToLuaExport 
{
    ...
    static void GenLuaFields()
    {
        ...
        if (HasDefaultConstructor() || (!isStaticClass && typeof(MonoBehaviour).IsAssignableFrom(type)))
        {
            sb.AppendFormat("\t\t\tnew LuaField(\"{0}\", get_{0}, null),\r\n", "isNil");
        }
    ...
    static void GenLuaMethods()
    {
        ...
        if(!isStaticClass)
        {
            sb.AppendLine("\t\t\tnew LuaMethod(\"unbind\", Lua_UnBind),");
        }
    ...
    static void GenIndexFunc()
    {
        ...
        if (HasDefaultConstructor() || (!isStaticClass && typeof(MonoBehaviour).IsAssignableFrom(type)))
        {
            ...
            CheckObjectNull();
            sb.AppendLine("\t\t{");
            sb.AppendLine("\t\t\tLuaScriptMgr.Push(L, true);");
            sb.AppendLine("\t\t}");
            sb.AppendLine("\t\telse {");
            sb.AppendLine("\t\t\tLuaScriptMgr.Push(L, false);");
            sb.AppendLine("\t\t}");
            sb.AppendLine();
            sb.AppendLine("\t\treturn 1;");
            sb.AppendLine("\t}");
    ...
    static void GenUnBindFunc()
    {
        if ( isStaticClass) return;
        sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
        sb.AppendLine("\tstatic int Lua_UnBind(IntPtr L)");
        sb.AppendLine("\t{");
        sb.AppendLine("\t\tLuaScriptMgr.__gc(L);\r\n");
        sb.AppendLine("\t\treturn 0;");
        sb.AppendLine("\t}");
